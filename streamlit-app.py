...
st.markdown("<p class='sub-header'>Select your symptoms to get a prediction</p>", unsafe_allow_html=True)
...
def load_model_and_features():
    ...
    try:
        ...
        features = [
            ...
            "involuntary urination", "pain during intercourse", "frequent urination", 
            "lower abdominal pain", "vaginal discharge", "blood in urine", 
            "hot flashes", "intermenstrual bleeding", "hand or finger pain", 
            "wrist pain", "hand or finger swelling", "arm pain", "wrist swelling", 
            "arm stiffness or tightness", "arm swelling", 
            ...
            "acne or pimples", "dry lips", "facial pain", "mouth ulcer", 
            "skin growth", "eye deviation", "diminished vision", "double vision", 
            "cross-eyed", "symptoms of eye", "pain in eye", "eye moves abnormally", 
            "abnormal movement of eyelid", "foreign body sensation in eye", 
            "irregular appearing scalp", "swollen lymph nodes", "back pain", 
            "neck pain", "low back pain", "pain of the anus", 
            "pain during pregnancy", "pelvic pain", "impotence", 
            "infant spitting up", "vomiting blood", "regurgitation", 
            "burning abdominal pain", "restlessness", "symptoms of infants", 
            "wheezing", "peripheral edema", "neck mass", "ear pain", 
            "jaw swelling", "mouth dryness", "neck swelling", "knee pain", 
            "foot or toe pain", "bowlegged or knock-kneed", "ankle pain", 
            "bones are painful", "knee weakness", "elbow pain", "knee swelling", 
            "skin moles", "knee lump or mass", "weight gain", 
            "problems with movement", "knee stiffness or tightness", 
            "leg swelling", "foot or toe swelling", "heartburn", 
            "smoking problems", "muscle pain", "infant feeding problem", 
            "recent weight loss", "problems with shape or size of breast", 
            "underweight", "difficulty eating", "scanty menstrual flow", 
            "vaginal pain", "vaginal redness", "vulvar irritation", "weakness", 
            ...
            "shoulder pain", "shoulder stiffness or tightness", 
            "shoulder weakness", "arm cramps or spasms", "shoulder swelling", 
            "tongue lesions", "leg cramps or spasms", "abnormal appearing tongue", 
            ...
            "leg stiffness or tightness", "difficulty breathing", "rib pain", 
            "joint pain", "muscle stiffness or tightness", "pallor", 
            "hand or finger lump or mass", "chills", "groin pain", "fatigue", 
            "abdominal distention", "regurgitation.1", "symptoms of the kidneys", 
            "melena", "flushing", "coughing up sputum", "seizures", 
            "delusions or hallucinations", "shoulder cramps or spasms", 
            "joint stiffness or tightness", "pain or soreness of breast", 
            "excessive urination at night", "bleeding from eye", 
            "rectal bleeding", "constipation", "temper problems", "coryza", 
            "wrist weakness", "eye strain", "hemoptysis", "lymphedema", 
            "skin on leg or foot looks infected", "allergic reaction", 
            "congestion in chest", "muscle swelling", "pus in urine", 
            "abnormal size or shape of ear", "low back weakness", "sleepiness", 
            "apnea", "abnormal breathing sounds", "excessive growth", 
            "elbow cramps or spasms", "feeling hot and cold", 
            ...
            "irregular appearing nails", "itching of skin", "hurts to breath", 
            ...
            "irregular belly button", "swollen tongue", "lip sore", "vulvar sore", 
            ...
            "knee cramps or spasms", "premature ejaculation", "leg weakness", 
            "posture problems", "bleeding in mouth", "tongue bleeding", 
            ...
            "elbow lump or mass", "muscle weakness", "throat redness", 
            "joint swelling", "tongue pain", "redness in or around nose", 
            "wrinkles on skin", "foot or toe weakness", 
...
with col1:
    ...
    # Create a search box to filter symptoms
    search_term = st.text_input("Type to search for symptoms", "")
    
    # Filter symptoms based on search
    filtered_symptoms = [s for s in all_symptoms if search_term.lower() in s.lower()] if search_term else all_symptoms
    
    # If the list is empty after filtering, show a message
    if not filtered_symptoms:
        st.warning("No symptoms match your search. Try a different term.")
    
    # Use multiselect with the filtered symptoms
...
with col2:
    ...
    if predict_button or selected_symptoms:
            ...
            with st.spinner("Analyzing symptoms..."):
                try:
                    ...
                    if hasattr(model, 'predict_proba'):
                        ...
                        top_indices = np.argsort(probabilities)[-3:][::-1]  # Get top 3 indices
                        ...
                        st.markdown(f"<h2>Likely Disease: {predicted_disease}</h2>", unsafe_allow_html=True)
                        ...
                        for disease, prob in zip(top_diseases, top_probs):
                            if prob > 0.05:  # Only show diseases with >5% probability
                                st.markdown(f"- **{disease}** (Confidence: {prob*100:.1f}%)")
                    else:
                        # If model doesn't have predict_proba, just show the prediction
                        st.markdown(f"<h2>Predicted Disease: {predicted_disease}</h2>", unsafe_allow_html=True)
...
with st.sidebar:
    st.image("https://via.placeholder.com/150x150.png?text=Medical", width=150)
